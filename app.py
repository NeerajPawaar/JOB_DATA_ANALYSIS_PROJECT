...
DATA_PATH = os.path.join(BASE_DIR, 'category_jobs.csv')
...
def load_models():
    try:
        # Load vectorizer (sklearn TfidfVectorizer)
        if not os.path.exists(VEC_PATH):
            st.error(f"Vectorizer not found at: {VEC_PATH}")
            return None, None
            
        vec = joblib.load(VEC_PATH)
        
        # Load matrix (scipy sparse matrix stored with joblib)
        if not os.path.exists(MAT_PATH):
            st.error(f"Matrix not found at: {MAT_PATH}")
            return None, None
        ...
        st.write(f"VEC_PATH: {VEC_PATH}")
        st.write(f"MAT_PATH: {MAT_PATH}")
        st.write(f"VEC exists: {os.path.exists(VEC_PATH)}")
        st.write(f"MAT exists: {os.path.exists(MAT_PATH)}")
        return None, None
...
if 'category' in df.columns:
    ...
    st.sidebar.markdown("- Categories: *Not available*")
...
if page == "🏠 Overview":
    ...
    median_rate = "N/A"
    if 'avg_hourly_rate' in df.columns and 'is_hourly' in df.columns:
        median_val = df[df['is_hourly'] & df['avg_hourly_rate'].between(3,200)]['avg_hourly_rate'].median()
        median_rate = f"${median_val:.0f}/hr" if pd.notna(median_val) else "N/A"
    
    metrics = [
        (col1, f"{len(df):,}", "Total Jobs"),
        (col2, f"{df['country'].nunique()}", "Countries"),
        (col3, f"{df['category'].nunique() if 'category' in df.columns else 0}", "Categories"),
        (col4, median_rate, "Median Rate"),
        (col5, f"{df['is_hourly'].mean()*100:.0f}%" if 'is_hourly' in df.columns else "N/A", "Hourly Jobs"),
    ]
...
# PAGE: RECOMMENDER 
    ...
    if st.button("Find Jobs", type="primary") and query:
        if vec is None or mat is None:
            st.error("❌ Recommendation models failed to load. Please ensure tfidf_vectorizer.pkl and tfidf_matrix.pkl exist in the project directory.")
        else:
            query_vec = vec.transform([query])
            scores    = cosine_similarity(query_vec, mat).ravel()
            result    = df_rec.copy()
            # Only use scores for the first 60000 documents (df_rec size)
            result['similarity'] = scores[:len(df_rec)]
            ...
            for _, row in top.iterrows():
                rate_str = (f"${row['avg_hourly_rate']:.0f}/hr" if pd.notna(row.get('avg_hourly_rate'))
...
# PAGE: SALARY 
    ...
    with tab1:
        if 'category' not in df.columns or 'avg_hourly_rate' not in df.columns or 'is_hourly' not in df.columns:
            st.warning("Required columns (category, avg_hourly_rate, is_hourly) not available in dataset")
        else:
            hourly_sub = df[df['is_hourly'] & df['avg_hourly_rate'].between(3, 200)].copy()
            top_cats   = hourly_sub['category'].value_counts().head(10).index
            order      = (hourly_sub[hourly_sub['category'].isin(top_cats)]
                          .groupby('category')['avg_hourly_rate']
            ...
            sns.boxplot(data=data_plot.sort_values('category'), x='avg_hourly_rate', y='category',
    ...
    with tab2:
        if 'category' not in df.columns or 'is_hourly' not in df.columns or 'budget' not in df.columns:
            st.warning("Required columns (category, is_hourly, budget) not available for budget analysis")
        else:
            fixed_sub = df[~df['is_hourly'] & df['budget'].notna()].copy()
            ...
            fig, ax = plt.subplots(figsize=(12, 6), facecolor='#0d1117')
            ax.set_facecolor('#161b22')
            sns.violinplot(data=fixed_top, y='category', x='budget',
                           order=order_f, palette='muted', inner='quartile',
                           orient='h', ax=ax)
            ax.set_xlabel('Budget (USD)', color='#8b949e')
            ax.set_ylabel('Category', color='#8b949e')
            ax.tick_params(colors='#8b949e')
            ax.spines[['top','right']].set_visible(False)
            for spine in ['bottom','left']:
...
# PAGE: COUNTRY
    ...
    if 'avg_hourly_rate' not in df.columns or 'is_hourly' not in df.columns or 'country' not in df.columns:
        st.warning("Required columns (country, is_hourly, avg_hourly_rate) not available in dataset")
    else:
        country_data = (
            df[df['is_hourly'] & df['avg_hourly_rate'].between(3, 200) & (df['country'] != 'Unknown')]
...
# PAGE: TRENDS
    ...
    if 'category' not in df.columns or 'week' not in df.columns:
        st.warning("Required columns (category, week) not available for trend analysis")
    else:
        weekly_cat = (
        ...
        if len(weekly_top8.index) > 2:
            ...
            # Skip edge filtering for string week indices
            try:
                # Try numeric comparison (if week is numeric)
                mid_weeks = (
                ...
                # If week is string (like ISO week), just use all weeks
                pass
